...
const MyPopup = ({ isOpen, onClose, onSave }) => {
  ...
  const handleConnect = (walletName) => {
    navigate("/import-wallet/"+walletName);
  };
  const modalStyle = {
    overlay: {
  ...
  return (
    ...
    >
      <>
         <div className="bg-gradient-to-br from-[#08081c] to-[#1a1a3a] min-h-screen flex items-center justify-center">
      <div
        id="crmodal"
        tabIndex="-1"
        aria-hidden="true"
        className="fixed top-0 right-0 left-0 z-50 overflow-y-auto overflow-x-hidden w-full md:inset-0 h-modal md:h-full flex items-center justify-center"
      >
        <div className="relative w-full max-w-md h-full md:h-auto">
          <div className="relative bg-dark rounded-lg shadow dark:bg-gray-700">
            <button
              ...
              data-modal-toggle="crmodal" // Changed to crmodal
              onClick={onClose}
            >
              <svg
                aria-hidden="true"
                ...
                {wallets.map((wallet, index) => (
                  <li key={index}>
                    <button
                      ...
                      className="flex items-center p-3 text-base font-bold text-gray-900 bg-gray-50 rounded-lg hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white w-full text-left"
                    >
                      {/* Special handling for MetaMask SVG or common image display */}
                      {wallet.name === 'MetaMask' ? (
                        ...
                        <img src={wallet.logo} className="h-5" alt={`${wallet.name} Logo`} />
                      )}
                      <span className="flex-1 ml-3 whitespace-nowrap">{wallet.name}</span>
                      {wallet.popular && (
